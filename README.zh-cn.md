...
- [go-jira](#go-jira)
  ...
  - [配置说明](#配置说明)
    ...
    - [使用示例](#使用示例)
      ...
      - [显示版本](#显示版本)
      - [使用自定义环境文件](#使用自定义环境文件)
  - [Jira issue 状态转移](#jira-issue-状态转移)
...
`go-jira schema` 会打印完整的命令与标志树，让代理程序（或脚本）无需爬取
...
这也是读取构建 commit 的推荐方式——`--version`（刻意只输出单一 semver 字符串）
已不再打印 commit。
