...
// create, update, get, transition list/execute, sprints, boards, link): load the
// env file, resolve the config, and require a valid base URL. These commands
// have no --ref, so they use requireBaseURL rather than the run-specific
// validateConfig.
...
// every Jira-talking subcommand (search, create, update, get, transition
// list/execute, sprints, boards, link). It mirrors the block in runWhoami:
// resolve the authenticator by the usual priority, validate it, then build an
// authenticated jira.Client against config.baseURL. Callers are expected to
// have already validated the base URL (via requireBaseURL) and loaded the env
// file.
